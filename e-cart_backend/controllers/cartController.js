...
exports.addToCart = async (req, res) => {
  ...
  try {
    ...
    if (products) {
      ...
      products.save();
      res.status(200).json("Product details updated in cart");
    } else {
      ...
      res.status(200).json("Product added successsfully to cart");
    }
  } catch (error) {
...
exports.incrementCartCount = async (req, res) => {
  ...
  try {
    ...
    if (product) {
      ...
      res.status(401).json("Product not found in cart");
    }
  } catch (error) {
    res.status(404).json(error);
  }
